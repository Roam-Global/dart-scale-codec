...
class GenericCall extends GeneralStruct {
  ...
  static dynamic get runtimeMetadata =>
      RuntimeConfigration().runtimeMetadata.obj;
  ...
  static Map<String, dynamic> extractIndex(Map<String, dynamic> json) {
    ...
    var module_index = isV12orLater
        ? metadataModules
            .firstWhere((m) => m.name.val == module_name,
                orElse: onModuleNotFound)
            .index
            .val
    ...
    var function_index =
        module.calls.obj.objects.indexWhere((f) => f.name.val == function_name);
    if (function_index == -1) {
      onFunctionNotFound();
    }
  ...
  GenericCall.fromJson(Map<String, dynamic> json)
      : super.fromJson(extractIndex(json)) {
    var args = json['args'];
    for (dynamic arg in callArgs) {
